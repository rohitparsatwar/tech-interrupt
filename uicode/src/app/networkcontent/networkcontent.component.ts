...
export class NetworkcontentComponent implements OnInit {
  ...
  getPartnerDetailsURL(partnerName: string){
    const getPartnerDetailsURL = "http://localhost:8080/pmaas/service/" + partnerName;
  ...
  getNetworkDetailsURL(){
    const getNetworkDetailsURL = "http://localhost:8080/pmaas/networks/" + this.networkName + "/details";
  ...
  getNetworkFeedsURL(){
    this.lastTimestamp = new Date();
    const getNetworkDetailsURL = "http://localhost:8080/pmaas/networks/" + this.networkName + "/feeds?startIndex=0";
  ...
  postFeed(feedText: string){
    const getAddFeedURL = "http://localhost:8080/pmaas/networks/" + this.networkName + "/feeds/"+feedText + "/partner/" + this.partnerData.name;
